...
export default function HomeExperience() {
  ...
  useEffect(() => {
    ...
    const initVanta = () => {
      ...
      if (!window.VANTA?.NET || vantaEffect) return;
      ...
      <section id="home" className="relative min-h-screen overflow-hidden px-6 pt-24 md:px-12">
        ...
        <div className="grid min-h-[80vh] items-center gap-10 lg:grid-cols-2">
          <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.8 }}>
            <motion.p initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="mb-4 text-sm uppercase tracking-[0.35em] text-cyan-300">
              K R I S H
            </motion.p>
        ...
        <div className="absolute bottom-8 left-1/2 h-14 w-8 -translate-x-1/2 rounded-full border border-white/30 p-1">
          <motion.div animate={{ y: [0, 24, 0] }} transition={{ repeat: Infinity, duration: 1.8 }} className="mx-auto h-2 w-2 rounded-full bg-cyan-300" />
        </div>
      </section>
          ...
          {skills.map((skill) => (
            <div key={skill.name} className="glass-card rounded-2xl p-4">
              ...
              <div className="h-2 rounded-full bg-white/10">
                <motion.div initial={{ width: 0 }} whileInView={{ width: `${skill.value}%` }} viewport={{ once: true }} className="h-full rounded-full bg-gradient-to-r from-cyan-300 to-violet-400" />
              </div>
            </div>
          ))}
        </div>
            ...
            <form onSubmit={handleFormSubmit} className="mx-auto max-w-3xl rounded-3xl border border-white/10 bg-black/25 p-6 backdrop-blur-xl md:p-8">
              ...
              {submitStatus && (
                <motion.div
                  ...
                  exit={{ opacity: 0, y: -10 }}
                  className={`mb-4 mt-4 rounded-lg p-3 text-center text-sm font-medium ${
              ...
              <div className="mt-6 flex flex-wrap items-center justify-center gap-4">
                <motion.button
                  ...
                  className={`glow rounded-full px-8 py-3 text-sm font-semibold transition-all ${
                    isSubmitting 
